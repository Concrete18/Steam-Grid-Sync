...
from utils.add_image import add_images
...
class SteamGrid:
    ...
    custom_grid_path = Path(cfg.get("Settings", "CUSTOM_GRID_PATH"))
    ...
    @staticmethod
    def info_print(image: SteamImage, action_type: str):
        """
    ...
    def get_images(self) -> list[SteamImage]:
        """
        ...
        for image_name in os.listdir(self.custom_grid_path):
            ...
            steam_image = SteamImage(image_path, self.steam_grid_path)
    ...
    def update_images(self, images_to_update: list[SteamImage]):
        updated = 0
    ...
    def sync(self) -> None:
        ...
        plural = " is" if update_total == 1 else "s are"
        msg = f"\n{update_total} Image{plural} queued for update\n"
        if update_total <= 10:
            print(msg)
            for img in images_to_update:
    ...
    def add_grid_images(self):
        """
        Allows adding multiple new Steam Grid images.
        """
        while True:
            add_images(self.custom_grid_path)
            msg = "\nDo you want to add another set of images?\n"
            if not yes_or_no(msg):
                break
        self.sync()
    ...
    def game_library_actions(self) -> None:
        ...
        choices = [
            ("Run Sync", self.sync),
            ("Add New Steam Grid Images", self.add_grid_images),
